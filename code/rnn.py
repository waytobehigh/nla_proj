...
class RNNModel (object):
    def __init__(self, params):
        ...
        self.rnn_cell = None
        # feature
        self.x = tf.compat.v1.placeholder("float", [None, params.time_steps, params.input_size])
        # label
        #self.y = tf.placeholder("float", [None, params.output_size])
        self.y = tf.compat.v1.placeholder("float", [None, params.output_size])
        ...
        config = tf.compat.v1.ConfigProto(device_count={'GPU' : int(params.gpu_flag)})
    ...
    def set_cell(self, params):
        if params.cell == "LSTM":
            self.rnn_cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(
                    num_units=params.num_units
                    )
        elif params.cell == "RNN":
            self.rnn_cell = tf.compat.v1.nn.rnn_cell.BasicRNNCell(
                    num_units=params.num_units
                    )
    ...
    def build(self, params):
        ...
        last_b = tf.compat.v1.get_variable("last_b", initializer=tf.keras.backend.truncated_normal([params.output_size], stddev=0.1))
        ...
        # get RNN cell output
        output, states = tf.compat.v1.nn.static_rnn(self.rnn_cell, x, dtype=np.float32)
	    # Apply Dropout
        ...
        self.normalize_vars = [v for v in tf.compat.v1.trainable_variables() if 'Householder' in v.name]
    ...
    def train(self, params, train_x, train_y, test_x, test_y):
        ...
        normalize_op = [tf.compat.v1.assign(v,tf.nn.l2_normalize(tf.linalg.band_part(v,0,-1),1)) for v in self.normalize_vars]
        ...
        train_error = []
        test_error = []
        #to save epochs and send it to main.py
        epochs_range = []
        iterations = 0
        time_used = 0
        ...
        for epoch in range(self.init_epoch, params.num_epochs):
            ...
            if epoch == self.init_epoch:
                ...
                epochs_range.append(self.init_epoch)
            ...
            for batch_begin in range(0, len(train_x), params.batch_size):
                ...
                if batch_index % (num_batches//params.display_epoch_num) == 0:
                    ...
                    #fix
                    #grad_value.append()
                    epochs_range.append(self.init_epoch+float(iterations)/num_batches)
            ...
            if np.isnan(train_error[-1]) or np.isinf(train_error[-1]) or np.isnan(test_error[-1]) or np.isinf(test_error[-1]):
                ...
                break
        #save in usefull format
        #self.save_data_to_file(train_error,test_error,grad_value)
        print("Optimization Finished!")

        return train_error, test_error, epochs_range
